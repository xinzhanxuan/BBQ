...
function CreateSnow(snowBox, src, style) {
  this.snowBox = document.getElementsByClassName('snow')[0];//找到容器
  this.snowStyle = Math.ceil(Math.random() * style);//雪花类型[1,2]
  this.maxLeft = this.snowBox.offsetWidth - Math.random() * 5 ;//运动范围
  this.maxTop = this.snowBox.offsetHeight - Math.random() * 5 ;
...
function goSnow(snowBox, src, num, style) {
  ...
  makeSnowtime = setInterval(function () {
...
window.onload = function () {
  var snowBox = document.getElementsByClassName('snow')[0];//雪花容器
    src = 'img/bj/snow.png',//雪花图基本命名<图片名就是snow+1/2/3/4...>
    num = 150,//雪花数量
    style = 2;//图片种类数
  goSnow(snowBox, src, num, style);
  window.onresize = function () {
    // clearInterval(makeSnowtime);
    // goSnow(snowBox, src, num, style);
  }
};